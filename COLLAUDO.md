...
| 16 | 🟡 | Pulsanti "+ Aggiungi", checklist spuntata | **La coda del #14: due colori chiari restano sotto soglia**, tutti e due lasciati apposta perché cambiano il peso di elementi molto ripetuti e volevano una decisione, non una sostituzione. `--color-add-text` (`#b8703a`) è la scritta dei ~19 pulsanti tratteggiati "+ Aggiungi…": **3,3-3,9:1**, si chiude cambiando **una riga sola** in `index.css` (`#9e5a20` passerebbe). `#b7a586` è la voce di checklist già spuntata, barrata, a **2,40:1** (`ChecklistRow.tsx:46`): lì lo sbiadito è voluto, quindi è una scelta di forma più che un difetto — o si scurisce accettando che il "fatto" pesi di più, o si lascia | 05/08 |
