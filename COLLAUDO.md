...
| 6 | 🟠 | Aggiunta a schermata Home | **Fatto il codice, manca la prova sul telefono — per questo è ancora qui.** C'è `manifest.webmanifest` (nome, `display: standalone`, `theme_color` crema `#f2ede3` come il fondo dell'app, non il corallo, che lassù diventerebbe una striscia staccata), le tre icone PNG con `purpose` separati — 192 e 512 `any`, una 512 `maskable` a parte, mai la scorciatoia `"any maskable"` — e in `index.html` il collegamento al manifest, il `theme-color` e le **cinque righe iOS**, che sono quelle che decidono davvero: senza `apple-touch-icon` (180×180, senza trasparenza, o iOS ci mette un fondo nero) l'iPhone si prende uno screenshot della pagina come icona; la barra dell'indirizzo la toglie `apple-mobile-web-app-capable`, non `display: standalone`; il nome sotto l'icona viene da `apple-mobile-web-app-title`. Icona provvisoria: il fenicottero attuale — la decisione sull'identità è rimandata a dopo il 14/08. **Resta da provare "Aggiungi a schermata Home" su iPhone e su Android** (blocco 5): finché non è passata lì, questa riga non si chiude | 04/08 |
